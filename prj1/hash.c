...
/*
-functionality: implement this in yuur own way and describe it in the document.
-parameter: integer that will be hashed
-return value: hash value of integer i
그냥 구현만 함, 사용 안 해도 됨*/
    unsigned hash = (unsigned)i;
    hash += ~(hash << 15); //왼쪽 시프트 후 비트 반전
    hash ^= (hash >> 9);  //오른쪽 시프트
    hash += (hash << 3);   //왼쪽 시프트
    hash ^= (hash >> 6);   //오른쪽 시프트
    hash += ~(hash << 11); //왼쪽 시프트 후 비트 반전
    hash ^= (hash >> 8);  //최종 오른쪽 시프트
    return hash;
}
