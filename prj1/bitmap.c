...
//////////////////////////////
struct bitmap *bitmap_expand(struct bitmap *bitmap, int extra_bits) {
  /*-functionality: expand the given bitmap to the size(backward expansion)
  -parameter: pointer of bitmap that you want to expand and the required siz of it
  return value: pointer of expanded bitmap if succeed, NULL if fail 

  */
    size_t new_bit_cnt = bitmap->bit_cnt + extra_bits;
    size_t old_byte_cnt = byte_cnt(bitmap->bit_cnt);
    size_t new_byte_cnt = byte_cnt(new_bit_cnt);
    // 새 비트맵 할당
    struct bitmap *new_bitmap = malloc(sizeof(struct bitmap));
    new_bitmap->bit_cnt = new_bit_cnt;
    new_bitmap->bits = calloc(new_byte_cnt, sizeof(unsigned char));
    // 기존 데이터 복사
    memcpy(new_bitmap->bits, bitmap->bits, old_byte_cnt);
    return new_bitmap;
}
